...
contract DirectSettlementModule_directSettleBatch is SettlementModule_Integration_Test {
    /*//////////////////////////////////////////////////////////////
                               LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    function test_directSettleBatch_Success() public {
        ...
        vm.startPrank(users.charlie.account);
        IDirectSettlementModule(address(adapter)).directSettleBatch(data.ordersWithSigs, data.destAmounts);
    ...
    function test_directSettleBatch_SuccessWithMixedTokens() public {
        ...
        vm.startPrank(users.charlie.account);
        IDirectSettlementModule(address(adapter)).directSettleBatch{ value: data.destAmounts[1] }(
    ...
    function test_directSettleBatch_RevertsWhen_OneSrcTokenTransferFails() public {
        ...
        vm.startPrank(users.alice.account);
        MTK.approve(address(address(adapter)), 0); // Remove approval
        vm.stopPrank();
    ...
    function test_directSettleBatch_RevertsWhen_OneDestTokenTransferFails() public {
        ...
        vm.startPrank(users.charlie.account);
        DAI.approve(address(address(adapter)), data.destAmounts[0] + data.destAmounts[1]); // Approve less than required
        vm.expectRevert();
        IDirectSettlementModule(address(adapter)).directSettleBatch(data.ordersWithSigs, data.destAmounts);
    }
}
