...
contract FillableSwapSettlementModule_feesFillable is SettlementModule_Integration_Test {
    /*//////////////////////////////////////////////////////////////
                               LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    function test_swapSettleFillable_WithFees() public {
        ...
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillable(
    ...
    function test_swapSettleFillableBatch_WithFees() public {
        ...
        vm.prank(users.charlie.account);
        uint256[] memory fillPercentages = new uint256[](3);
        fillPercentages[0] = FillableStorageLib.HUNDRED_PERCENT;
        fillPercentages[1] = FillableStorageLib.HUNDRED_PERCENT;
        fillPercentages[2] = FillableStorageLib.HUNDRED_PERCENT;
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillableBatch(
            ordersWithSigs, executorDataArray, address(executor), fillPercentages
        );
    ...
    function test_batchWithdrawAllFees() public {
        TestData memory data1 = createTestData(100, 50, address(MTK), address(DAI), 1); // 1% partner fee, 0.5%
            // executor fee
        ...
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillable(
            data1.orderWithSig, abi.encode(data1.executorData), address(executor), FillableStorageLib.HUNDRED_PERCENT
        );
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillable(
            data2.orderWithSig, abi.encode(data2.executorData), address(executor), FillableStorageLib.HUNDRED_PERCENT
        );
        vm.stopPrank();
    ...
    function test_settleOrdersWithDifferentFeePercentages() public {
        ...
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillable(
            data3.orderWithSig, abi.encode(data3.executorData), address(executor), FillableStorageLib.HUNDRED_PERCENT
        );
        vm.stopPrank();
    ...
    function test_settleOrdersWithDifferentTokens() public {
        ...
        vm.startPrank(users.charlie.account);
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillable(
            dataDAI.orderWithSig,
            abi.encode(dataDAI.executorData),
            address(executor),
            FillableStorageLib.HUNDRED_PERCENT
        );
        IFillableSwapSettlementModule(address(adapter)).swapSettleFillable(
            dataWETH.orderWithSig,
            abi.encode(dataWETH.executorData),
            address(executor),
            FillableStorageLib.HUNDRED_PERCENT
        );
        vm.stopPrank();
    ...
    function test_swapSettleFillable_WithZeroFees() public {
    ...
    function test_swapSettleFillable_PartialFillWithFees() public {
        TestData memory data = createTestData(100, 50, address(MTK), address(DAI), 1); // 1% partner fee, 0.5% executor
            // fee
        uint256 fillPercent = 5000; // 50%
        data.executorFee = 0;
        uint256 expectedFillAmount = 492_525;
        uint256 expectedPartnerFee = ((expectedFillAmount - 1) * data.partnerFee) / 10_000;

        // Update executor data to reflect partial fill
        data.executorData = ExecutorData(
            StepData(
                abi.encodeWithSignature("approve(address,uint256)", address(dex), data.srcAmount), data.order.srcToken
            ),
            StepData(
                abi.encodeWithSignature(
                    "swap(address,address,uint256,uint256,address)",
    ...
    function test_swapSettleFillable_WithSurplus() public {
        TestData memory data = createTestData(100, 50, address(MTK), address(DAI), 1); // 1% partner fee, 0.5% executor
            // fee
    ...
    function test_swapSettleFillable_WithoutSurplus() public {
        TestData memory data = createTestData(100, 50, address(MTK), address(DAI), 1); // 1% partner fee, 0.5% executor
            // fee
        data.order.expectedDestAmount = data.order.destAmount * 110 / 100; // Set expected amount 10% higher
        data.orderWithSig = signOrder(data.order);
        ...
        uint256 expectedPartnerFee = (actualAmount - data.executorFee - 1) * data.partnerFee / 10_000;
    ...
    function test_swapSettleFillableBatch_WithMixedSurplus() public {
        uint256[] memory partnerFeePercentages = new uint256[](3);
        partnerFeePercentages[0] = 100; // 1%
        partnerFeePercentages[1] = 200; // 2%
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            fillPercentages[i] = FillableStorageLib.HUNDRED_PERCENT;
    ...
    function invariant_totalFeesNeverExceedActualAmount() public {
        // This test ensures that the sum of all fees (partner, protocol, executor)
        // never exceeds the actual amount received
    ...
    function invariant_partnerFeesNeverExceedSpecifiedPercentage() public {
        // This test ensures that partner fees never exceed the specified percentage
        TestData memory data = createTestData(200, 100, address(MTK), address(DAI), 1); // 2% partner fee, 1% executor
            // fee
        data.orderWithSig = signOrder(data.order);
    ...
    function invariant_protocolFeesOnlyCollectedWithSurplus() public {
        // This test ensures that protocol fees are only collected when there's a surplus
        TestData memory data = createTestData(100, 50, address(MTK), address(DAI), 1); // 1% partner fee, 0.5% executor
            // fee
    ...
    function invariant_filledAmountNeverExceedsOrderAmount() public {
        // This test ensures that the filled amount never exceeds the original order amount
        TestData memory data = createTestData(100, 50, address(MTK), address(DAI), 1);
        data.orderWithSig = signOrder(data.order);
        ...
        uint256 filledAmount = IFillableSwapSettlementModule(address(adapter)).swapFilledAmount(data.order);

        assert(filledAmount <= data.order.srcAmount);
    }

    function invariant_executorFeeNeverExceedsSpecifiedAmount() public {
        // This test ensures that the executor fee never exceeds the specified amount
        TestData memory data = createTestData(100, 50, address(MTK), address(DAI), 1); // 1% partner fee, 0.5% executor
            // fee
        data.orderWithSig = signOrder(data.order);
