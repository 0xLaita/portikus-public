...
contract SwapSettlementModule_Integration_swapSettleBatch is SettlementModule_Integration_Test {
    /*//////////////////////////////////////////////////////////////
                               LIBRARIES
    //////////////////////////////////////////////////////////////*/
    ...
    function createTestData() internal returns (TestData memory data) {
        ...
        for (uint256 i = 0; i < data.numOrders; i++) {
            ...
            // Transfer MTK to alice and approve adapter
            vm.startPrank(users.admin.account);
            MTK.transfer(users.alice.account, srcAmount);
            vm.startPrank(users.alice.account);
            MTK.approve(address(adapter), type(uint256).max);
    ...
    function test_swapSettleBatch_Success() public {
        ...
        ISwapSettlementModule(address(adapter)).swapSettleBatch(
            data.ordersWithSigs, data.executorDataArray, address(executor)
        );
    ...
    function test_swapSettleBatch_RevertsWhen_BeforeStepFailed() public {
        // Setup
        TestData memory data = createTestData();
        ...
        for (uint256 i = 0; i < data.numOrders; i++) {
            ExecutorData memory executorData = abi.decode(data.executorDataArray[i], (ExecutorData));
            executorData.beforeCalldata = StepData(abi.encodeWithSignature("revert()"), address(this));
            data.executorDataArray[i] = abi.encode(executorData);
        }

        // Attempt to settle batch with failing beforeStep
        vm.startPrank(users.charlie.account);
    ...
    function test_swapSettleBatch_RevertsWhen_MainStepFailed() public {
        // Setup
        TestData memory data = createTestData();
        ...
        for (uint256 i = 0; i < data.numOrders; i++) {
            ExecutorData memory executorData = abi.decode(data.executorDataArray[i], (ExecutorData));
            executorData.mainCalldata = StepData(abi.encodeWithSignature("revert()"), address(this));
            data.executorDataArray[i] = abi.encode(executorData);
        }

        // Attempt to settle batch with failing mainStep
        vm.startPrank(users.charlie.account);
        vm.expectRevert(ExecutionFailed.selector);
        ISwapSettlementModule(address(adapter)).swapSettleBatch(
            data.ordersWithSigs, data.executorDataArray, address(executor)
    ...
    function test_swapSettleBatch_RevertsWhen_AfterStepFailed() public {
        // Setup
        TestData memory data = createTestData();
        ...
        for (uint256 i = 0; i < data.numOrders; i++) {
            ExecutorData memory executorData = abi.decode(data.executorDataArray[i], (ExecutorData));
            executorData.afterCalldata = StepData(abi.encodeWithSignature("revert()"), address(this));
            data.executorDataArray[i] = abi.encode(executorData);
        }

        // Attempt to settle batch with failing afterStep
        vm.startPrank(users.charlie.account);
        vm.expectRevert(ExecutionFailed.selector);
        ISwapSettlementModule(address(adapter)).swapSettleBatch(
            data.ordersWithSigs, data.executorDataArray, address(executor)
